...
In order to evaluate the performance of SecureWilly, we will first calculate its computational complexity.
...
It becomes evident, that the performance of SecureWilly over each benchmark cannot be evaluated using time, as every test plan has different computational complexity.
...
It appears that there is a threshold in runs, which represents the minimum number of runs which have to be executed, until all of the project's profiles stabilize over runs. The threshold for media streaming as shown in the graph is at run 3. After the threshold, it takes three runs for SecureWilly to finish its execution.
...
\begin{figure}[h!]
  \centering
   \includegraphics[width=0.70\linewidth]{./figures/nextcloud/rulesthreshold.png}
